...
**maakie-brainlab** は、非AI研究者のための **根拠付きローカル知識ワークベンチ** です。ユーザーは資料を入れ、質問し、根拠を確認する。この最短導線を主軸にしつつ、検証や証跡化の仕組みは裏側に残します。
...
### 1) 通常UIを使うだけ（`make gate1` は不要）

`make gate1` はCI向け検証です。通常UIの確認だけなら以下の2ターミナルで十分です。
...
- 通常導線は `資料 / 質問 / 根拠` の3面です。
- 運用・実験・追跡系の画面は `/ops` に退避しています。
- Phase 1 時点では表面整理を優先しており、詳細な資料管理や質問フローの統合は次段階で進めます。
