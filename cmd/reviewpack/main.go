...
func runPack(args []string) {
	...
	runCmd(packDir, "git", "status", ">", filepath.Join(packDir, "01_status.txt"))
	
	nCommits := "5" // default
	if fs.NArg() > 0 {
		nCommits = fs.Arg(0)
	}
	runCmd(packDir, "git", "log", "-n", nCommits, "--stat", ">", filepath.Join(packDir, "10_git_log.txt"))
	runCmd(packDir, "git", "diff", "HEAD~"+nCommits, "HEAD", ">", filepath.Join(packDir, "11_git_diff.patch"))
	...
	if !*skipEval {
        ...
        if err != nil { log.Printf("[WARN] failed to write skip log: %v", err) }
	}
	...
	// 7. Manifest & Checksums (Deterministic)
...
func runVerify(args []string) {
    ...
    // Allowed exceptions: CHECKSUMS.sha256, 40_self_verify.log
	err = filepath.WalkDir(verifyRoot, func(path string, d fs.DirEntry, err error) error {
        ...
        if rel == "CHECKSUMS.sha256" || rel == "40_self_verify.log" {
            return nil
        }
		if !validFiles[rel] {
...
## まずやる
./reviewpack verify . (or bash VERIFY.sh if provided)
`
...
echo "[INFO] For strict verification run 'reviewpack verify .'"
`
...
func runSelfVerify(dir string) {
    ...
    fmt.Fprintln(f, "Self-verify passed (generated by Go).")
}
