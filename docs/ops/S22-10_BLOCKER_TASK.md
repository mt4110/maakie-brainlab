...
- [ ] 3) milestone “S22-10” の 取得 or 作成
    - MID をログ（OK: milestone_number=…）
- [ ] 4) PR #89 に milestone 付与 → 再観測
- [ ] 5) PR #90 に milestone 付与 → 再観測
- [ ] 6) milestone_required の再実行（必要時のみ）
    - rerun できなければ SKIP（理由1行）→ UI手動へ
- [ ] 7) merge guard（#89 → #90 の順）
    - #89 dry-run → --merge
    - #90 dry-run → --merge（※#89 merge 後に）
- [ ] 8) 追加：変更ファイルが200超なら Copilot review を挟む（best-effort）
    - changedFiles 観測して判断（重くない）
    - UIでCopilot review要求（できない/制限ならSKIP理由1行）
- [ ] 9) マイルストーン close（best-effort / 失敗はSKIP）
