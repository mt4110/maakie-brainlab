...
## 成果物（このステップで必ず出す）
- docs/ops/S15_08_PLAN.md（本書：TBDゼロ）
- docs/ops/S15_08_TASK.md（実行順固定・TBDゼロ）
- Dependency matrix の更新（docs/ops/S15_07_10_DEPENDENCY_MATRIX.md）
- “PR分割ルール”の明文化（PLANにもTASKにも1箇所は必ず書く）

## 重要ルール（決定論・監査耐性）
- 曖昧語を残さない：TBD / maybe / might / later を残したら未完
...
3) Fill S15_08 PLAN/TASK (remove TBD)
4) Update matrix (DependsOn, Touch, NoTouch, Gate)
5) Decide PR naming conventions
6) Gates (verify-only)

## Done条件（監査用）
- docs/ops/S15_08_PLAN.md に TBD が0件
- docs/ops/S15_08_TASK.md に TBD が0件
