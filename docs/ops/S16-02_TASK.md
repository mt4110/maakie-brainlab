...
## Policy Scan (MUST)
- [ ] `bash -lc 'cd "$(git rev-parse --show-toplevel)"; set -u; PAT="$(printf "%c%c%c%c%c%c%c" 102 105 108 101 58 47 47)"; git grep -n -F -- "$PAT"; st=$?; case "$st" in 0) echo "[FAIL] forbidden file URL found" >&2; exit 1 ;; 1) echo "[OK] none" ;; *) echo "[ERROR] git grep failed (exit=$st)" >&2; exit "$st" ;; esac'`
