...
- [x] 手順が 順序固定（並べ替え不要で実行できる）
- [x] skip に has 必ず1行理由 を残すルールが入っている
- [x] error は その場で終了 のルールが入っている
...
## 5) 差分確認（想定外混入を殺す）
- [x] git diff --stat が 3点のみ（PLAN/TASK/MATRIX）
- [x] git diff を目視し、機械的ルールとDone条件が読めることを確認

## 6) ゲート（Eco）
- [x] reviewpack submit --mode verify-only を実行
- [x] docs-only でない差分が出た場合 → error (スコープ外)

## 7) コミット & PR
- [x] ブランチ：s15-08-kickoff-impl-split-v1
- [x] コミット：docs(s15-08): lock dependencies + PR slicing rules
- [x] PR本文に「Done条件」「変更対象3点」「verify-only結果」を記載
