...
- [x] Print repo root + status (manual read)
  - Command:
...
- [x] Inventory current python wiring (no changes)
...
- [x] Delete `.venv` (local) then run canonical bootstrap (timebox)
- [x] `make test` PASS
- [x] `git status -sb` clean
- [x] `go run cmd/reviewpack/main.go submit --mode verify-only` PASS

### 6) Evidence + PR
- [x] Capture latest `.local/prverify/prverify_*.md` into `docs/evidence/prverify/`
- [x] Create PR body in SOT/証拠スタイル（ガチガチ版 + 短い版併記）
