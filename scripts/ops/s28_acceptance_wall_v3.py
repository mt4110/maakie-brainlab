...
DEFAULT_CASES_FILE = "docs/ops/S28-07_ACCEPTANCE_CASES_V3.json"
...
def build_markdown(payload: Dict[str, Any]) -> str:
    ...
    lines: List[str] = []
    lines.append("# S28-07 Acceptance Wall v3 (Latest)")
    lines.append("")
    ...
    lines.append("")
    lines.append("```md")
    lines.append("### S28-07 Acceptance Wall v3")
    lines.append(f"- status: {summary.get('status', '')}")
    lines.append(f"- reason_code: {summary.get('reason_code', '')}")
    lines.append(f"- acceptance: {summary.get('passed_cases', 0)}/{summary.get('total_cases', 0)}")
    lines.append(f"- critical_failed: {summary.get('critical_failed', 0)}")
    lines.append(f"- severity_failed: {summary.get('severity_failed', {})}")
    lines.append(f"- artifact: docs/evidence/s28-07/{payload.get('artifact_names', {}).get('json', '')}")
    lines.append("```")
    lines.append("")
    return "\n".join(lines)
...
def write_failure(out_dir: Path, repo_root: Path, cases_path: Path, reason_code: str, message: str) -> Dict[str, Any]:
    payload: Dict[str, Any] = {
        "schema_version": "s28-acceptance-wall-v3",
        ...
        "artifact_names": {"json": "acceptance_wall_v3_latest.json", "md": "acceptance_wall_v3_latest.md", "run_dir": ""},
    }
    out_json = out_dir / "acceptance_wall_v3_latest.json"
    out_md = out_dir / "acceptance_wall_v3_latest.md"
...
def main() -> int:
    ...
    run_dir, meta, events = make_run_context(repo_root, tool="s28-acceptance-wall-v3", obs_root=args.obs_root)
    ...
    payload: Dict[str, Any] = {
        ...
        "cases": rows,
        "artifact_names": {
            "json": "acceptance_wall_v3_latest.json",
            "md": "acceptance_wall_v3_latest.md",
