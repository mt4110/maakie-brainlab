...
DEFAULT_OUT_DIR = "docs/evidence/s28-06"
...
def build_markdown(payload: Dict[str, Any]) -> str:
    ...
    lines: List[str] = []
    lines.append("# S28-06 Reliability Soak v2 (Latest)")
    lines.append("")
...
def main() -> int:
    ...
    run_dir, meta, events = make_run_context(repo_root, tool="s28-reliability-soak-v2", obs_root=args.obs_root)
    ...
    if status == "FAIL":
        ...
        emit("WARN", f"soak v2 WARN reason={reason_code}", events)
    else:
        emit("OK", "soak v2 PASS", events)

    payload: Dict[str, Any] = {
        "schema_version": "s28-reliability-soak-v2",
    ...
    out_json = out_dir / "reliability_soak_v2_latest.json"
    out_md = out_dir / "reliability_soak_v2_latest.md"
