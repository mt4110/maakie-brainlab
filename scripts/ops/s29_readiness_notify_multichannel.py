...
DEFAULT_OUT_DIR = "docs/evidence/s29-03"
...
def channel_env_suffix(channel: str) -> str:
...
def aggregate_channel_delivery(channel_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    total = len(channel_results)
    attempted = sum(1 for row in channel_results if bool(row.get("attempted")))
    sent = sum(1 for row in channel_results if bool(row.get("sent")))
    attempt_count = sum(int(row.get("attempt_count", 0) or 0) for row in channel_results)
    failed = sum(1 for row in channel_results if bool(row.get("attempted")) and not bool(row.get("sent")))
    not_configured = sum(1 for row in channel_results if str(row.get("delivery_state") or "") == "NOT_ATTEMPTED")
    ...
    rate = None
    if attempted > 0:
        rate = round(sent / float(attempted), 4)
    ...
    return {
        ...
        "not_configured_channels": not_configured,
        "attempt_count": attempt_count,
        "delivery_rate": rate,
        "delivery_state": state,
        "sent": sent == total and total > 0,
...
def build_markdown(payload: Dict[str, Any]) -> str:
    ...
    for row in channels:
        lines.append(
            "- channel=`{}` state=`{}` attempted=`{}` sent=`{}`".format(
                row.get("channel", ""),
...
def main() -> int:
    ...
    parser.add_argument("--channels", default="#ops-release,#ops-oncall")
    parser.add_argument("--channel", default="#ops-release")
    parser.add_argument("--webhook-env", default="S29_READINESS_WEBHOOK_URL")
    parser.add_argument("--channel-webhook-prefix", default="S29_READINESS_WEBHOOK_URL_")
    ...
    run_dir, meta, events = make_run_context(repo_root, tool="s29-readiness-notify-multichannel", obs_root=args.obs_root)
    ...
    if status != "FAIL":
        if not args.send:
            ...
            for channel in channels:
                suffix = channel_env_suffix(channel)
                channel_env = f"{str(args.channel_webhook_prefix)}{suffix}"
                webhook_url = str(os.environ.get(channel_env, "") or os.environ.get(str(args.webhook_env), "") or "")
                msg = compose_message(channel, readiness, schedule)
                ...
                notify_payload = {
                    ...
                    "text": msg,
                    "meta": {
                        "tool": "s29-readiness-notify-multichannel",
                ...
                row["delivery_rate"] = compute_delivery_rate(
                ...
                channel_results.append(row)
                if bool(row.get("sent")):
                    emit(
                        "OK",
                        f"notification delivered channel={channel} status={row.get('http_status')} attempts={row.get('attempt_count', 0)}",
                        events,
                    )
                else:
                    emit(
                        "WARN",
                        f"notification failed channel={channel} status={row.get('http_status')} attempts={row.get('attempt_count', 0)} err={row.get('error')}",
                        events,
                    )
    ...
    payload: Dict[str, Any] = {
        ...
        "inputs": {
            ...
            "channel_webhook_prefix": str(args.channel_webhook_prefix),
            "send": bool(args.send),
            "include_response_tail": bool(args.include_response_tail),
            "max_retries": int(args.max_retries),
            "retry_backoff_sec": float(args.retry_backoff_sec),
        ...
        "artifact_names": {
            "json": "readiness_notify_multichannel_latest.json",
            "md": "readiness_notify_multichannel_latest.md",
        },
    }

    out_json = out_dir / "readiness_notify_multichannel_latest.json"
    out_md = out_dir / "readiness_notify_multichannel_latest.md"
