...
Goal:
- Index S27-01..S27-07 latest artifacts.
...
DEFAULT_OUT_DIR = "docs/evidence/s27-08"
DEFAULT_HISTORY_FILE = "evidence_trend_history.json"
...
def build_markdown(payload: Dict[str, Any]) -> str:
    ...
    lines: List[str] = []
    lines.append("# S27-08 Evidence Trend Index v2 (Latest)")
    lines.append("")
    ...
    lines.append("")
    lines.append("```md")
    lines.append("### S27-08 Evidence Trend Index v2")
    lines.append(f"- status: {summary.get('status', '')}")
    lines.append(f"- present_missing: {summary.get('present_count', 0)}/{summary.get('missing_count', 0)}")
    lines.append(f"- pass_warn_fail: {summary.get('pass_count', 0)}/{summary.get('warn_count', 0)}/{summary.get('failed_count', 0)}")
    lines.append(f"- history_size: {summary.get('history_size', 0)}")
    lines.append(f"- artifact: docs/evidence/s27-08/{payload.get('artifact_names', {}).get('json', '')}")
    lines.append("```")
    lines.append("")
    return "\n".join(lines)
...
def main() -> int:
    ...
    run_dir, meta, events = make_run_context(repo_root, tool="s27-evidence-trend-index", obs_root=args.obs_root)
    ...
    history_doc = {
        "schema_version": "s27-evidence-trend-history-v1",
        "updated_at_utc": now,
        "snapshots": snapshots,
    }
    ...
    payload: Dict[str, Any] = {
        "schema_version": "s27-evidence-trend-index-v2",
        "captured_at_utc": now,
        ...
        "artifact_names": {"json": "evidence_trend_index_latest.json", "md": "evidence_trend_index_latest.md", "history": str(args.history_file)},
    }

    out_json = out_dir / "evidence_trend_index_latest.json"
    out_md = out_dir / "evidence_trend_index_latest.md"
