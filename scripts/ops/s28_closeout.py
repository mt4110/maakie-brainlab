#!/usr/bin/env python3
"""
S28-10 closeout artifact generator.

Goal:
- Freeze S28 Before/After, unresolved risks, and S29 handoff.
- Use S28 SLO readiness as the final decision source.
"""
...
DEFAULT_OUT_DIR = "docs/evidence/s28-10"
DEFAULT_READINESS = "docs/evidence/s28-09/slo_readiness_v2_latest.json"
DEFAULT_INDEX = "docs/evidence/s28-08/evidence_trend_index_v3_latest.json"
...
def build_markdown(payload: Dict[str, Any]) -> str:
    ...
    lines: List[str] = []
    lines.append("# S28-10 Closeout (Latest)")
    lines.append("")
    ...
    lines.append(f"- artifact: docs/evidence/s28-10/{payload.get('artifact_names', {}).get('json', '')}")
    lines.append("```")
    lines.append("")
    return "\n".join(lines)
...
def write_failure(
    ...
    payload: Dict[str, Any] = {
        "schema_version": "s28-closeout-v1",
...
def main() -> int:
    ...
    run_dir, meta, events = make_run_context(repo_root, tool="s28-closeout", obs_root=args.obs_root)
