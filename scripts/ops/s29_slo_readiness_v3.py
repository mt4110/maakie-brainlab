...
DEFAULT_OUT_DIR = "docs/evidence/s29-09"
...
def build_gate_rows(docs: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for phase in ["S29-01", "S29-02", "S29-03", "S29-04", "S29-05", "S29-06", "S29-07", "S29-08"]:
        doc = docs.get(phase, {})
        ...
        hard.append(
            {
                ...
                "threshold": recovery_success_hard,
                "rule": "value >= hard",
            }
        )
    elif recovery_success_rate < recovery_success_soft:
        soft.append(
            {
                "metric": "recovery_success_rate",
                "value": recovery_success_rate,
                "threshold": recovery_success_soft,
                "rule": "value >= soft",
            }
        )
...
def build_waiver_context(
    *,
    ...
    d01_recovery_slo = dict(d01.get("recovery_slo", {}))
    ...
    recovery_success_env_gap = provider_env_gap and str(d01_recovery_slo.get("level") or "") in {
        "SOFT_WARN",
        "HARD_BREACH",
        "INSUFFICIENT_SAMPLE",
    }
...
def apply_metric_waivers(
    ...
    *,
    ...
    for row in hard:
        ...
        elif metric == "recovery_success_rate" and _to_bool(context.get("recovery_success_env_gap")):
            waiver_code = WAIVER_RECOVERY_SUCCESS_ENV_GAP
            waiver_note = "recovery success-rate degradation is dominated by provider environment gap."
...
def main() -> int:
    ...
    parser.add_argument("--skip-rate-soft", type=float, default=0.15)
    parser.add_argument("--skip-rate-hard", type=float, default=0.30)
    parser.add_argument("--unknown-ratio-soft", type=float, default=0.03)
    parser.add_argument("--unknown-ratio-hard", type=float, default=0.15)
    parser.add_argument("--acceptance-pass-rate-soft", type=float, default=0.97)
    ...
    recovery_success_rate = to_float(dict(d01.get("recovery_slo", {})).get("success_rate", 0.0), 0.0)
