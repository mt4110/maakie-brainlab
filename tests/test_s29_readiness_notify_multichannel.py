...
def _load_module():
    ...
    path = root / "scripts" / "ops" / "s29_readiness_notify_multichannel.py"
    spec = importlib.util.spec_from_file_location("s29_readiness_notify_multichannel", path)
...
class S29ReadinessNotifyTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.m = _load_module()
    ...
    def test_parse_channels(self):
        out = self.m.parse_channels("#ops,#ops,#ml")
        self.assertEqual(out, ["#ops", "#ml"])

    def test_aggregate_channel_delivery_partial(self):
        summary = self.m.aggregate_channel_delivery(
            [
                {"attempted": True, "sent": True, "attempt_count": 1, "delivery_state": "SENT"},
                {"attempted": True, "sent": False, "attempt_count": 2, "delivery_state": "FAILED"},
            ]
        )
    ...
    def test_send_requested_without_webhook_keeps_not_attempted(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            env = os.environ.copy()
            env.pop("S29_READINESS_WEBHOOK_URL", None)
