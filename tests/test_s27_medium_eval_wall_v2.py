...
class S27MediumEvalWallV2Tests(unittest.TestCase):
    ...
    def test_validate_config_rejects_invalid_tag_min_counts(self):
        cfg = {
            "schema_version": "s27-medium-eval-wall-v2",
            "dataset_id": "x",
            "cases_path": "a.jsonl",
            ...
            "tag_min_counts": {"provider": "oops"},
            "taxonomy_map": {"provider": ["provider"]},
        }
        ok, reason = self.m.validate_config(cfg)
        self.assertFalse(ok)
        self.assertIn("tag_min_counts.provider invalid", reason)

    def test_validate_contract_handles_invalid_tag_min_value(self):
        dist = {"total_cases": 3, "must_answer_true": 2, "must_answer_false": 1, "must_cite_true": 2, "tag_counts": {"provider": 2}}
        contract = {"min_cases": 2, "max_cases": 10, "min_must_answer_true": 2, "min_must_answer_false": 1, "min_must_cite_true": 2}
        errs = self.m.validate_contract(dist, contract, {"provider": "oops"})
        self.assertIn("tag provider min invalid", errs)
