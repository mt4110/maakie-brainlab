...
def _load_module():
    ...
    path = root / "scripts" / "ops" / "s29_slo_readiness_v3.py"
    spec = importlib.util.spec_from_file_location("s29_slo_readiness_v3", path)
...
class S29SLOReadinessV3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.m = _load_module()
    ...
    def test_build_gate_rows(self):
        rows = self.m.build_gate_rows(
            {
                "S29-01": {"summary": {"status": "PASS"}},
                "S29-02": {"summary": {"status": "WARN"}},
                "S29-03": {},
                "S29-04": {"summary": {"status": "PASS"}},
                "S29-05": {"summary": {"status": "PASS"}},
                "S29-06": {"summary": {"status": "PASS"}},
                "S29-07": {"summary": {"status": "PASS"}},
                "S29-08": {"summary": {"status": "PASS"}},
            }
        )
        self.assertEqual(len(rows), 8)
        self.assertFalse(next(r for r in rows if r["phase"] == "S29-03")["passed"])
    ...
    def test_build_waiver_context(self):
        context = self.m.build_waiver_context(
            d01={"trend": {"dominant_skip_cause": "env", "env_skip_rate": 1.0}, "recovery_slo": {"level": "SOFT_WARN"}},
    ...
    def test_apply_metric_waivers(self):
        hard = [
            ...
            {"metric": "recovery_success_rate", "value": 0.0, "threshold": 0.5, "rule": "value >= hard"},
        ...
        self.assertEqual(len(out["hard"]), 0)
        self.assertEqual(len(out["waived_hard"]), 5)
        self.assertEqual(len(out["soft_from_hard"]), 5)
