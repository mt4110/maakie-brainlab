...
class S27ReleaseReadinessScheduleTests(unittest.TestCase):
    ...
    def test_status_from_readiness_blocked(self):
        out = self.m.status_from_readiness("BLOCKED")
        self.assertEqual(out["status"], "FAIL")
        self.assertEqual(out["reason_code"], self.m.REASON_READINESS_BLOCKED)
    ...
    def test_main_skip_run_honors_blocked_primary_artifact(self):
        repo_root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory() as td:
            out_dir = Path(td) / "out"
            primary_artifact = Path(td) / "primary.json"
            primary_artifact.write_text(
                json.dumps({"summary": {"readiness": "BLOCKED", "blocked_total": 2}}),
                encoding="utf-8",
            )
            cp = subprocess.run(
                [
            ...
            payload = json.loads((out_dir / "release_readiness_schedule_latest.json").read_text(encoding="utf-8"))
            self.assertEqual(payload["summary"]["status"], "FAIL")
            self.assertEqual(payload["summary"]["reason_code"], self.m.REASON_READINESS_BLOCKED)
